...
def build_app() -> FastAPI:
    ...
    # CORS добавляется ПОСЛЕ auth → в Starlette это делает его внешним слоем
    # (middleware-стек LIFO: добавленный последним выполняется первым на входе).
    # Так CORS обрабатывает preflight OPTIONS ДО проверки токена — иначе браузерный
    # preflight (без кастомных заголовков) ловил бы 401 от SidecarAuthMiddleware.
    #
    # Figma-плагин рендерит UI в sandboxed iframe с Origin: "null", поэтому в
    # allow_origins должен присутствовать литерал "null". Кредами (cookies) мы не
    # пользуемся — авторизация к сайдкару идёт через X-Phygital-Sidecar-Token,
    # поэтому allow_credentials=False и можно безопасно перечислить нужные origin'ы.
    # Сам сайдкар слушает только loopback и защищён токеном — CORS тут не граница
    # безопасности, а лишь разрешение для браузерного клиента Figma.
    app.add_middleware(
        CORSMiddleware,
        allow_origins=["null", "https://www.figma.com"],
