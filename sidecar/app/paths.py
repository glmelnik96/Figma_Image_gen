...
def docs_dir() -> Path:
    """Каталог с brand system-prompt .md документами (бандл в репозитории
    сайдкара, app/paths.py → app/ → sidecar/docs)."""
    return Path(__file__).resolve().parent.parent / "docs"


def brand_docs_cache() -> Path:
    """Журнал brand-документов (filename → file_obj_id + sha256 + uploaded_at)."""
    return resolve_app_data() / "brand_docs.json"
