...
from app.workflows.brand import BRAND_PIPELINES
...
class JobCreate(BaseModel):
    ...
    # pipeline: опциональный brand-пайплайн ("brand_t2i" / "brand_i2i"). Если
    # задан — job исполняется как оркестрация Gemini Text (brand system-prompt)
    # → Nano Banana, а node_id служит только для семафоров (используем 94).
...
def _state_to_dict(s: JobState) -> dict:
    ...
    params_out = {
        k: v for k, v in (s.params or {}).items() if k not in ("_init_files", "_pipeline")
    }
    return {
        "job_id": s.job_id,
        "node_id": s.node_id,
...
async def create_job(
    ...
    if body.pipeline is not None and body.pipeline not in BRAND_PIPELINES:
        raise HTTPException(
            status_code=400,
            detail={"error": "unknown_pipeline", "pipeline": body.pipeline,
                    "allowed": sorted(BRAND_PIPELINES)},
        )
    ...
    if body.pipeline:
        params["_pipeline"] = body.pipeline
