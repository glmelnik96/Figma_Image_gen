...
from app.workflows.base import ProgressCallback, Workflow, _normalize_progress
...
# ── Flash-fallback helpers (общие для brand_t2i / brand_i2i) ──────────────
def is_transient_gemini_failure(job: GenerationJob) -> bool:
    """True если Phygital вернул transient-error (status=error / 504 / server
    error), при котором имеет смысл повторить с Flash-моделью.
