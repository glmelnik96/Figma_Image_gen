...
class JobRunner:
    ...
    async def _do_run(self, job_id: str, state, workflow_class) -> None:
        ...
        try:
            ...
            # чтобы клиент видел реальный прогресс, а не 0%→100%.
            ...
            # Brand-пайплайн (Cloud.ru): Gemini Text c brand system-prompt →
            # Nano Banana. Это оркестрация двух Phygital-тасков, не одиночная
            # нода — обрабатываем до обычной node-диспетчеризации.
            pipeline = params.pop("_pipeline", None)
            if pipeline:
                from app.workflows.brand import run_brand_pipeline

                entries = slot_entries.get("init_img") or slot_entries.get("images") or []
                init_ids = [e.file_obj_id for e in entries]
                init_dims = [
                ...
                gen_job: GenerationJob = await run_brand_pipeline(
                    pipeline, client,
                    params=params, init_img_ids=init_ids, init_img_dims=init_dims,
                    pct_cb=_on_progress,
                )
                await self._finalize(job_id, gen_job)
                return
            ...
            if state.node_id == 94 and slot_entries.get("init_img"):
            ...
            elif state.node_id == 98 and slot_entries.get("images"):
                # GPT Image (98) img2img: same shape as node 94 but the slot is
                # "images" and the workflow exposes it via self._init_img_ids /
                # self._init_img_dims (see GPTImageWorkflow._images_input). The
                # generic path below would silently drop the refs because
                # build_payload ignores an `images` kwarg.
                entries = slot_entries["images"]
                dims = [
                    {"height": e.height, "width": e.width}
                    for e in entries
                    if e.height is not None and e.width is not None
                ...
                if len(dims) != len(entries):
                    raise RuntimeError(
                        f"images: AssetCache entries missing height/width "
                        f"({len(dims)}/{len(entries)}); cache stale, re-add files"
                    )
                workflow = workflow_class(client)
                workflow._init_img_ids = params.pop("images")
                workflow._init_img_dims = dims
            else:
                workflow = workflow_class(client)
            await self.registry.update_status(job_id, status="running")
            ...
            gen_job = await workflow.run(**params)
            await self._finalize(job_id, gen_job)
    ...
    async def _finalize(self, job_id: str, gen_job: GenerationJob) -> None:
        """Скачать результат завершённой job или пометить failed."""
        if gen_job.status == "completed":
            await self.registry.update_status(
                job_id, status="downloading", task_id=gen_job.job_id,
            )
            out_dir = self.downloads_root / job_id
            paths = await self._download_urls(
                urls=gen_job.result_urls, out_dir=out_dir,
            )
            await self.registry.update_status(
